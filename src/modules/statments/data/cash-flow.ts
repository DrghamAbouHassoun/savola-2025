export const cashFlowStatementsEn = {
  headers: ["", "Note", "2025", "2024"],
  raws: [
    {
      data: ["Cash flows from operating activities", "", "", ""],
      isBold: true,
    },
    {
      data: ["Profit for the year", "", "940,499", "9,915,352"],
      isBold: false,
    },
    {
      data: ["Adjustments for:", "", "", ""],
      isBold: true,
    },
    {
      data: ["Depreciation and amortisation", "", "1,198,359", "1,154,964"],
      isBold: false,
    },
    {
      data: ["Net finance cost", "22 & 41", "423,753", "1,030,227"],
      isBold: false,
    },
    {
      data: [
        "Share of results in equity-accounted",
        "",
        "",
        "",
      ],
      isBold: true,
    },
    {
      data: [
        "investees, net of zakat and tax",
        "10.1",
        "(51,775)",
        "(808,635)",
      ],
      isBold: false,
    },
    {
      data: ["Share based payment expense", "18", "8,804", "36,356"],
      isBold: true,
    },
    {
      data: ["Impairment charge, net", "40", "24,640", "390,409"],
      isBold: false,
    },
    {
      data: [
        "Loss on sale / write-off of property, plant and equipment and software",
        "",
        "30,313",
        "895",
      ],
      isBold: false,
    },
    {
      data: [
        "Gain on distribution of investment in equity-accounted investee",
        "16",
        "-",
        "(11,554,662)",
      ],
      isBold: false,
    },
    {
      data: [
        "Loss on derecognition of investment in equity-accounted investee",
        "1",
        "-",
        "139,001",
      ],
      isBold: false,
    },
    {
      data: [
        "(Gain) / loss on derecognition of subsidiaries",
        "22",
        "(33,958)",
        "1,422,268",
      ],
      isBold: false,
    },
    {
      data: ["Loss on FVTPL investments", "38", "4,764", "18,496"],
      isBold: false,
    },
    {
      data: ["Net impairment loss on financial assets", "13", "51,755", "1,987"],
      isBold: false,
    },
    {
      data: [
        "Provision for employee benefits liability",
        "24",
        "110,541",
        "111,776",
      ],
      isBold: false,
    },
    {
      data: ["Zakat (reversal) / expense, net", "29", "(217,425)", "350,010"],
      isBold: false,
    },
    {
      data: ["Income tax expense", "29", "117,005", "252,604"],
      isBold: false,
    },
    {
      data: ["", "", "2,607,275", "2,461,048"],
      isBold: true,
    },
    {
      data: ["Changes in:", "", "", ""],
      isBold: true,
    },
    {
      data: ["Inventories", "", "(546,134)", "(330,979)"],
      isBold: false,
    },
    {
      data: ["Trade receivables", "", "(59,465)", "(672,982)"],
      isBold: false,
    },
    {
      data: [
        "Prepayments and other receivables",
        "",
        "77,157",
        "(201,103)",
      ],
      isBold: false,
    },
    {
      data: ["Trade payables", "", "268,378", "532,126"],
      isBold: false,
    },
    {
      data: ["Accrued and other liabilities", "", "(422,649)", "582,577"],
      isBold: false,
    },
    {
      data: [
        "Cash generated from operating activities",
        "",
        "1,924,562",
        "2,370,687",
      ],
      isBold: true,
    },
    {
      data: [
        "Finance cost paid on loans and borrowings",
        "21.2",
        "(369,320)",
        "(768,056)",
      ],
      isBold: false,
    },
    {
      data: [
        "Finance cost paid on lease liabilities",
        "23",
        "(196,555)",
        "(178,404)",
      ],
      isBold: false,
    },
    {
      data: ["Zakat and income tax paid", "29", "(192,623)", "(202,347)"],
      isBold: false,
    },
    {
      data: ["Employee benefits liabilities paid", "24", "(97,410)", "(81,987)"],
      isBold: false,
    },
    {
      data: [
        "Net cash generated from operating activities",
        "",
        "1,068,654",
        "1,139,893",
      ],
      isBold: true,
    },
    {
      data: ["Cash flows from investing activities", "", "", ""],
      isBold: true,
    },
    {
      data: [
        "Acquisition of property, plant and equipment",
        "6",
        "(800,529)",
        "(733,345)",
      ],
      isBold: false,
    },
    {
      data: ["Acquisition of intangible assets", "8", "(57,971)", "(41,034)"],
      isBold: false,
    },
    {
      data: ["Purchase of Government Sukuks", "11.5", "(523,858)", "-"],
      isBold: false,
    },
    {
      data: [
        "Proceeds from sale of property, plant and equipment",
        "",
        "58,020",
        "136,304",
      ],
      isBold: false,
    },
    {
      data: ["Additions in investment in associate", "10", "-", "(59,965)"],
      isBold: false,
    },
    {
      data: ["Acquisition of subsidiary", "1", "-", "173,065"],
      isBold: false,
    },
    {
      data: ["Acquisition of non-controlling interest", "1", "(25,010)", "-"],
      isBold: false,
    },
    {
      data: ["Finance income from bank deposits", "", "145,677", "132,548"],
      isBold: false,
    },
    {
      data: [
        "Cash outflow due to derecognition of subsidiary",
        "22",
        "(29,265)",
        "(452,165)",
      ],
      isBold: false,
    },
    {
      data: [
        "Proceeds from disposal of subsidiary",
        "22",
        "473,587",
        "231,413",
      ],
      isBold: false,
    },
    {
      data: [
        "Dividend received from equity accounted investment",
        "10",
        "25,116",
        "368,540",
      ],
      isBold: false,
    },
    {
      data: ["Dividend received from FVOCI investment", "", "2,126", "-"],
      isBold: false,
    },
    {
      data: ["Term deposits placed", "", "(1,465,747)", "(1,385,315)"],
      isBold: false,
    },
    {
      data: ["Term deposits redeemed", "", "1,959,191", "1,193,861"],
      isBold: false,
    },
    {
      data: [
        "Net cash used in investing activities",
        "",
        "(238,663)",
        "(436,093)",
      ],
      isBold: true,
    },
    {
      data: ["Cash flows from financing activities", "", "", ""],
      isBold: true,
    },
    {
      data: [
        "Proceeds from long-term loans and borrowings",
        "21",
        "-",
        "976,535",
      ],
      isBold: false,
    },
    {
      data: [
        "Repayments of long-term loans and borrowings",
        "21",
        "(260,487)",
        "(5,782,930)",
      ],
      isBold: false,
    },
    {
      data: [
        "Net change in short term loans and borrowings",
        "",
        "(984,180)",
        "136,566",
      ],
      isBold: false,
    },
    {
      data: ["Right issue proceeds", "16", "-", "6,000,000"],
      isBold: false,
    },
    {
      data: [
        "Transaction cost related to right issue, capital reduction and distribution",
        "",
        "-",
        "(47,691)",
      ],
      isBold: false,
    },
    {
      data: ["Payment against put liability", "1", "(270,909)", "(64,842)"],
      isBold: false,
    },
    {
      data: [
        "Payment of lease liabilities – Principal",
        "23",
        "(529,588)",
        "(482,171)",
      ],
      isBold: false,
    },
    {
      data: [
        "Dividends paid to the owners of the Company",
        "",
        "(3,415)",
        "(2,533)",
      ],
      isBold: false,
    },
    {
      data: [
        "Dividends paid to non-controlling interests",
        "",
        "(72,492)",
        "(53,016)",
      ],
      isBold: false,
    },
    {
      data: [
        "Purchase of shares held under employees’ share based payment plan",
        "19",
        "(70,000)",
        "(33,348)",
      ],
      isBold: false,
    },
    {
      data: [
        "Net cash (used in) / generated from financing activities",
        "",
        "(2,191,071)",
        "646,570",
      ],
      isBold: true,
    },
    {
      data: [
        "Net change in cash and cash equivalents",
        "",
        "(1,361,080)",
        "1,350,370",
      ],
      isBold: true,
    },
    {
      data: [
        "Effect of movement in exchange rates on cash and cash Equivalents",
        "",
        "55,813",
        "(334,865)",
      ],
      isBold: false,
    },
    {
      data: [
        "Cash and cash equivalents at the beginning of the year",
        "",
        "2,147,964",
        "1,132,459",
      ],
      isBold: false,
    },
    {
      data: [
        "CASH AND CASH EQUIVALENTS AT THE END OF THE YEAR",
        "15",
        "842,697",
        "2,147,964",
      ],
      isBold: true,
    },
  ],
};

export const cashFlowStatementsAr = {
  "headers": [
    "",
    "إيضاحات",
    "2025م",
    "2024م"
  ],
  "raws": [
    {
      "data": [
        "التدفقات النقدية من الأنشطة التشغيلية",
        "",
        "",
        ""
      ],
      "isBold": true
    },
    {
      "data": [
        "الربح للسنة",
        "",
        "940,499",
        "9,915,352"
      ],
      "isBold": false
    },
    {
      "data": [
        "تعديلات على:",
        "",
        "",
        ""
      ],
      "isBold": true
    },
    {
      "data": [
        "استهلاك واطفاء",
        "",
        "1,198,359",
        "1,154,964"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي التكاليف التمويلية",
        "22 و 41",
        "423,753",
        "1,030,227"
      ],
      "isBold": false
    },
    {
      "data": [
        "حصة من نتائج شركات مستثمر فيها بطريقة حقوق الملكية، بعد خصم الزكاة والضرائب",
        "10.1",
        "(51,775)",
        "(808,635)"
      ],
      "isBold": false
    },
    {
      "data": [
        "مصروف برنامج أسهم الموظفني",
        "18",
        "8,804",
        "36,356"
      ],
      "isBold": false
    },
    {
      "data": [
        "مخصص خسارة الانخفاض في القيمة، بالصافي",
        "40",
        "24,640",
        "390,409"
      ],
      "isBold": false
    },
    {
      "data": [
        "خسارة/ (ربح) من بيع أو وشطب ممتلكات وآلات ومعدات و برامج",
        "",
        "30,313",
        "895"
      ],
      "isBold": false
    },
    {
      "data": [
        "ربح من توزيع شركة مستثمر فيها بطريقة حقوق الملكية",
        "16",
        "-",
        "(11,554,662)"
      ],
      "isBold": false
    },
    {
      "data": [
        "خسارة ناتجة عن إلغاء الاعتراف بالاستثمار في شركة مستثمر فيها بطريقة حقوق الملكية",
        "1",
        "-",
        "139,001"
      ],
      "isBold": false
    },
    {
      "data": [
        "ربح / (خسارة) ناتجة عن إلغاء الاعتراف بالشركات التابعة",
        "22",
        "(33,958)",
        "1,422,268"
      ],
      "isBold": false
    },
    {
      "data": [
        "خسارة من استثمارات بالقيمة العادلة من خلال الربح أو الخسارة",
        "38",
        "4,764",
        "18,496"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي خسارة انخفاض في قيمة الموجودات المالية",
        "13",
        "51,755",
        "1,987"
      ],
      "isBold": false
    },
    {
      "data": [
        "مخصص التزامات منافع الموظفني",
        "24",
        "110,541",
        "111,776"
      ],
      "isBold": false
    },
    {
      "data": [
        "(عكس) / مصروف الزكاة، بالصافي",
        "29",
        "(217,425)",
        "350,010"
      ],
      "isBold": false
    },
    {
      "data": [
        "مصروف ضريبة دخل",
        "29",
        "117,005",
        "252,604"
      ],
      "isBold": false
    },
    {
      "data": [
        "",
        "",
        "2,607,275",
        "2,461,048"
      ],
      "isBold": true
    },
    {
      "data": [
        "تغيريات في:",
        "",
        "",
        ""
      ],
      "isBold": true
    },
    {
      "data": [
        "مخزون",
        "",
        "(546,134)",
        "(330,979)"
      ],
      "isBold": false
    },
    {
      "data": [
        "ذمم المدينة التجارية",
        "",
        "(59,465)",
        "(672,982)"
      ],
      "isBold": false
    },
    {
      "data": [
        "مدفوعات مقدمًا وذمم مدينة أخرى",
        "",
        "77,157",
        "(201,103)"
      ],
      "isBold": false
    },
    {
      "data": [
        "ذمم دائنة تجارية",
        "",
        "268,378",
        "532,126"
      ],
      "isBold": false
    },
    {
      "data": [
        "مستحقات ومطلوبات أخرى",
        "",
        "(422,649)",
        "582,577"
      ],
      "isBold": false
    },
    {
      "data": [
        "النقد الناتج من الأنشطة التشغيلية",
        "",
        "1,924,562",
        "2,370,687"
      ],
      "isBold": true
    },
    {
      "data": [
        "تكلفة تمويلية مدفوعة على قروض وسلف",
        "21.2",
        "(369,320)",
        "(768,056)"
      ],
      "isBold": false
    },
    {
      "data": [
        "تكلفة تمويلية مدفوعة على مطلوبات عقود إيجار",
        "23",
        "(196,555)",
        "(178,404)"
      ],
      "isBold": false
    },
    {
      "data": [
        "زكاة وضريبة دخل مدفوعة",
        "29",
        "(192,623)",
        "(202,347)"
      ],
      "isBold": false
    },
    {
      "data": [
        "التزامات منافع موظفني مدفوعة",
        "24",
        "(97,410)",
        "(81,987)"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي النقد الناتج من الأنشطة التشغيلية",
        "",
        "1,068,654",
        "1,139,893"
      ],
      "isBold": true
    },
    {
      "data": [
        "التدفقات النقدية من الأنشطة الاستثمارية",
        "",
        "",
        ""
      ],
      "isBold": true
    },
    {
      "data": [
        "استحواذ على ممتلكات وآلات ومعدات",
        "6",
        "(800,529)",
        "(733,345)"
      ],
      "isBold": false
    },
    {
      "data": [
        "استحواذ على موجودات غير ملموسة",
        "8",
        "(57,971)",
        "(41,034)"
      ],
      "isBold": false
    },
    {
      "data": [
        "شراء صكوك حكومية",
        "11.5",
        "(523,858)",
        "-"
      ],
      "isBold": false
    },
    {
      "data": [
        "متحصلات من بيع ممتلكات وآلات ومعدات",
        "",
        "58,020",
        "136,304"
      ],
      "isBold": false
    },
    {
      "data": [
        "إضافات إلى استثمار في شركة زميلة",
        "10",
        "-",
        "(59,965)"
      ],
      "isBold": false
    },
    {
      "data": [
        "استحواذ على شركة تابعة",
        "1",
        "-",
        "173,065"
      ],
      "isBold": false
    },
    {
      "data": [
        "استحواذ على حقوق ملكية غير مسيطرة",
        "1",
        "(25,010)",
        "-"
      ],
      "isBold": false
    },
    {
      "data": [
        "إيرادات تمويلية من الودائع البنكية",
        "",
        "145,677",
        "132,548"
      ],
      "isBold": false
    },
    {
      "data": [
        "التدفق النقدي الخارج نتيجة إلغاء الاعتراف بشركة تابعة",
        "22",
        "(29,265)",
        "(452,165)"
      ],
      "isBold": false
    },
    {
      "data": [
        "متحصلات من بيع شركة تابعة",
        "22",
        "473,587",
        "231,413"
      ],
      "isBold": false
    },
    {
      "data": [
        "توزيعات أرباح مستلمة من شركات مستثمر فيها بطريقة حقوق الملكية",
        "10",
        "25,116",
        "368,540"
      ],
      "isBold": false
    },
    {
      "data": [
        "توزيعات ارباح من استثمارات بالقيمة العادلة من خلال الدخل الشامل الاخر",
        "",
        "2,126",
        "-"
      ],
      "isBold": false
    },
    {
      "data": [
        "ودائع لأجل مودعة",
        "",
        "(1,465,747)",
        "(1,385,315)"
      ],
      "isBold": false
    },
    {
      "data": [
        "ودائع لأجل مسرتدة",
        "",
        "1,959,191",
        "1,193,861"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي النقد المستخدم في الأنشطة الاستثمارية",
        "",
        "(238,663)",
        "(436,093)"
      ],
      "isBold": true
    },
    {
      "data": [
        "التدفقات النقدية من الأنشطة التمويلية",
        "",
        "",
        ""
      ],
      "isBold": true
    },
    {
      "data": [
        "متحصلات من قروض وسلف طويلة الأجل",
        "21",
        "-",
        "976,535"
      ],
      "isBold": false
    },
    {
      "data": [
        "سداد قروض وسلف طويلة الأجل",
        "21",
        "(260,487)",
        "(5,782,930)"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي التغير في قروض وسلف قصرية الأجل",
        "",
        "(984,180)",
        "136,566"
      ],
      "isBold": false
    },
    {
      "data": [
        "متحصلات من إصدار الحقوق",
        "16",
        "-",
        "6,000,000"
      ],
      "isBold": false
    },
    {
      "data": [
        "التكاليف المرتبطة بطرح حقوق الأولوية وتخفيض رأس المال والتوزيع",
        "",
        "-",
        "(47,691)"
      ],
      "isBold": false
    },
    {
      "data": [
        "دفعة مقابل التزام خيار البيع",
        "1",
        "(270,909)",
        "(64,842)"
      ],
      "isBold": false
    },
    {
      "data": [
        "مدفوعات الجزء الأساسي من مطلوبات عقود إيجار",
        "23",
        "(529,588)",
        "(482,171)"
      ],
      "isBold": false
    },
    {
      "data": [
        "توزيعات ارباح مدفوعة لمساهمي الشركة",
        "",
        "(3,415)",
        "(2,533)"
      ],
      "isBold": false
    },
    {
      "data": [
        "توزيعات أرباح مدفوعة لحقوق ملكية غير مسيطرة",
        "",
        "(72,492)",
        "(53,016)"
      ],
      "isBold": false
    },
    {
      "data": [
        "شراء أسهم محتفظ بها بموجب برنامج المدفوعات على أساس أسهم الموظفني",
        "19",
        "(70,000)",
        "(33,348)"
      ],
      "isBold": false
    },
    {
      "data": [
        "صافي النقد (المستخدم في) / الناتج من الأنشطة التمويلية",
        "",
        "(2,191,071)",
        "646,570"
      ],
      "isBold": true
    },
    {
      "data": [
        "صافي التغير في النقد وما في حكمه",
        "",
        "(1,361,080)",
        "1,350,370"
      ],
      "isBold": true
    },
    {
      "data": [
        "تأثري الحركة في أسعار صرف العمالت على النقد وما في حكمه",
        "",
        "55,813",
        "(334,865)"
      ],
      "isBold": false
    },
    {
      "data": [
        "النقد وما في حكمه في بداية السنة",
        "",
        "2,147,964",
        "1,132,459"
      ],
      "isBold": false
    },
    {
      "data": [
        "النقد وما في حكمه في نهاية السنة",
        "15",
        "842,697",
        "2,147,964"
      ],
      "isBold": true
    }
  ]
}